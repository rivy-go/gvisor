...
func (fstype FilesystemType) newFilesystem(vfsObj *vfs.VirtualFilesystem, creds *auth.Credentials) (*kernfs.Filesystem, *kernfs.Dentry) {
	...
	rootDentry := &kernfs.Dentry{}
	rootDentry.Init(root)
	...
	masterDentry := &kernfs.Dentry{}
	masterDentry.Init(master)

	// Add the master as a child of the root.
	links := root.OrderedChildren.Populate(rootDentry, map[string]*kernfs.Dentry{
		"ptmx": masterDentry,
	})
	root.IncLinks(links)
...
func (i *rootInode) allocateTerminal(creds *auth.Credentials) (*Terminal, error) {
	...
	// Linux always uses pty index + 2 as the inode id.
	slave.InodeAttrs.Init(creds, uint64(idx+2), linux.ModeCharacterDevice|0666)
	i.slaves[idx] = slave
