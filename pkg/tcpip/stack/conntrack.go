...
// transport protocols.
func (ct *ConnTrackTable) ConnTrackForPacket(pkt PacketBuffer, hook Hook, connType int) *connTrack {
...
func (ct *ConnTrackTable) SetNatInfo(pkt PacketBuffer, rt RedirectTarget, hook Hook) {
	conn := ct.ConnTrackForPacket(pkt, hook, getConnForPacket)
	if conn == nil {
		return
	}
...
func handlePacketOutput(pkt PacketBuffer, conn *connTrack, gso *GSO, r *Route) {
	netHeader := header.IPv4(pkt.NetworkHeader)
	tcpHeader := header.TCP(pkt.TransportHeader)
...
func (ct *ConnTrackTable) HandlePacket(pkt PacketBuffer, hook Hook, gso *GSO, r *Route) bool {
	...
	case Output:
		handlePacketOutput(pkt, conn, gso, r)
	}
