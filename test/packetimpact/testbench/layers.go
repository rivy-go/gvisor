...
type Layer interface {
	...
	// diff diffs the current Layer and the provided Layer. If either
	// Layer has a nil in a given field, that field is considered matching.
	// Otherwise, the values pointed to by the fields must match. The LayerBase is
	// ignored.
	diff(Layer) string
...
// diffLayer diffs two Layer structs while ignoring fields in which either input
// has a nil and also ignoring the LayerBase of the inputs.
func diffLayer(x, y Layer) string {
	if x == nil || y == nil {
		return ""
	}
	// opt ignores comparison pairs where either of the inputs is a nil.
	...
	return cmp.Diff(x, y, opt, cmpopts.IgnoreTypes(LayerBase{}))
}
...
func (l *Ether) diff(other Layer) string {
	return diffLayer(l, other)
}
...
func (l *IPv4) diff(other Layer) string {
	return diffLayer(l, other)
}
...
func (l *TCP) diff(other Layer) string {
	return diffLayer(l, other)
}
...
func (l *UDP) diff(other Layer) string {
	return diffLayer(l, other)
}
...
func (l *Payload) diff(other Layer) string {
	return diffLayer(l, other)
}
...
func (ls *Layers) diff(other Layers) string {
	...
	for i := 0; i < len(*ls) && i < len(other); i++ {
		if i < len(*ls) {
			if i < len(other) {
				allDiffs = append(allDiffs, diffLayer((*ls)[i], other[i]))
			} else {
				allDiffs = append(allDiffs, diffLayer((*ls)[i], nil))
			}
		} else {
			allDiffs = append(allDiffs, diffLayer(nil, other[i]))
		}
	}
