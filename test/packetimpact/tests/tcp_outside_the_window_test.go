...
func TestTCPOutsideTheWindow(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(fmt.Sprintf("%s sent with %v payload, %d bytes outside the window", tt.description, tt.payload, tt.seqNumOffset), func(t *testing.T) {
			dut := tb.NewDUT(t)
			defer dut.TearDown()
			...
			gotAck, err := conn.Expect(tb.TCP{Flags: tb.Uint8(header.TCPFlagAck)}, timeout)
