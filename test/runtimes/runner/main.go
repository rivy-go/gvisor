...
func runTests() int {
	...
	tests, cleanup, err := getTests(d, blacklist)
	if err != nil {
...
// index.
func getTests(d dockerutil.Docker, blacklist map[string]struct{}) ([]testing.InternalTest, func(), error) {
	// Wrap all cleanups, as they are returned.
	...
	// Copy in the proctor file, since we will use it as our payload.
	dir, _, err := dockerutil.PrepareFiles("test/runtimes/proctor/proctor")
	if err != nil {
		cleanupAll()
		return nil, nil, fmt.Errorf("PrepareFiles() failed: %v", err)
	}
	// XXX cleanups = append(cleanups, cleanup)

	// Run proctor with --pause flag to keep container alive forever.
	mountArg := dockerutil.MountArg(dir, "/proctor", dockerutil.ReadOnly)
	if err := d.Run(mountArg, *image, "/proctor/proctor", "--pause"); err != nil {
		cleanupAll()
		return nil, nil, fmt.Errorf("docker run failed: %v", err)
	}
	// XXX cleanups = append(cleanups, d.CleanUp)

	// Get a list of all tests in the image.
	list, err := d.Exec("/proctor/proctor", "--runtime", *lang, "--list")
	if err != nil {
		cleanupAll()
		return nil, nil, fmt.Errorf("docker exec failed: %v", err)
	}
	...
	if err != nil {
		cleanupAll()
		return nil, nil, fmt.Errorf("TestsForShard() failed: %v", err)
	}
		...
		itests = append(itests, testing.InternalTest{
			...
			F: func(t *testing.T) {
				...
				go func() {
					...
					output, err = d.Exec("/proctor/proctor", "--runtime", *lang, "--test", tc)
					close(done)
				}()
